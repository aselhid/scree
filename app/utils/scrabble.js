...
const isOneAxis = new_chars => {
  ...
  return indexToCoord(new_chars[0]).reduce(
    (acc, next, index) => acc || next === averages[index],
    false
  );
};
...
const getPlayedWords = (new_chars_index, table) => {
  ...
  new_chars_index.forEach(element => {
    ...
    while (queue.length > 0) {
      ...
      try {
        if (
          !horizontal.includes(current_index) &&
          table[current_i][current_j]
        ) {
...
const uniq = a => {
  ...
  return a.filter(
    element => (seen.hasOwnProperty(element) ? false : (seen[element] = true))
  );
};
